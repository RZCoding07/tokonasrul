...
class Pages extends BaseController
{
    ...
    {
        \Midtrans\Config::$serverKey = 'SB-Mid-server-hByEKJFm3rguW26PywuU7q51';
        \Midtrans\Config::$isProduction = false;
        \Midtrans\Config::$isSanitized = true;
        \Midtrans\Config::$is3ds = true;
     
        $harga = 0; 
        foreach($this->transaksiModel->where('kode_trx', $kode_trx)->findAll() as $d){
            $harga += $d['harga'] * $d['kuantitas']; 
        }

        $uri = service('uri');
        $params = array(
            'transaction_details' => array(
                'order_id' => $uri->getSegment(2),
                'gross_amount' => $harga,
            )
        );
